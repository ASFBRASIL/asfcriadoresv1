...
import {
  ...
  BarChart3, Filter, RefreshCw, ExternalLink, Image, Upload,
  FileText, Eye, Bell, type LucideIcon
} from 'lucide-react';
...
function StatCard({ title, value, icon: Icon, color = 'green' }: { title: string; value: number | string; icon: LucideIcon; color?: string }) {
  const bg: Record<string, string> = { green: 'bg-[var(--asf-green)]/10 text-[var(--asf-green)]', yellow: 'bg-[var(--asf-yellow)]/20 text-[var(--asf-yellow-dark)]', blue: 'bg-blue-50 text-blue-600', purple: 'bg-purple-50 text-purple-600', red: 'bg-red-50 text-red-600' };
  return (
    <div className="bg-white rounded-2xl p-5 border border-gray-100 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-sm text-[var(--asf-gray-medium)] mb-1">{title}</p>
          <p className="text-3xl font-bold text-[var(--asf-gray-dark)]">{typeof value === 'number' ? value.toLocaleString('pt-BR') : value}</p>
        </div>
    ...
    <div className="space-y-2.5">{data.map((item, i) => (
      <div key={i} className="flex items-center gap-3">
        ...
        <div className="flex-1 h-7 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-[var(--asf-green)] to-[var(--asf-green-light)] rounded-full" style={{ width: `${(item[valueKey] / max) * 100}%` }} />
        </div>
            ...
            {stats.criadoresPorMes.map((item: any, i: number) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-2">
                  <span className="text-xs font-semibold text-[var(--asf-green)]">{item.count}</span>
                  <div className="w-full bg-gradient-to-t from-[var(--asf-green)] to-[var(--asf-green-light)] rounded-t-lg" style={{ height: `${(item.count / mx) * 100}%`, minHeight: '8px' }} />
                  <span className="text-xs text-[var(--asf-gray-medium)]">{item.mes}</span>
                </div>
              );
            })}
          </div>
      ...
      {stats?.avaliacoesPorNota?.length > 0 && (
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          ...
          <div className="space-y-2">{stats.avaliacoesPorNota.map((item: any) => {
            const tot = stats.avaliacoesPorNota.reduce((s: number, a: any) => s + a.count, 0) || 1;
            return (
              <div key={item.nota} className="flex items-center gap-3">
                <div className="flex items-center gap-1 w-14"><Star className="w-4 h-4 fill-[var(--asf-yellow)] text-[var(--asf-yellow)]" /><span className="text-sm font-medium">{item.nota}</span></div>
                <div className="flex-1 h-5 bg-gray-100 rounded-full overflow-hidden"><div className="h-full bg-[var(--asf-yellow)] rounded-full" style={{ width: `${(item.count / tot) * 100}%` }} /></div>
                <span className="text-sm text-[var(--asf-gray-medium)] w-20 text-right">{item.count} ({Math.round((item.count / tot) * 100)}%)</span>
              </div>
            );
          })}</div>
        </div>
      ...
      <div className="bg-white rounded-2xl p-4 border border-gray-100">
        <div className="flex flex-wrap gap-3">
          <div className="flex-1 min-w-[200px] relative">
            ...
            <input placeholder="Buscar nome, email, cidade..." value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && doSearch()} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 focus:border-[var(--asf-green)] focus:ring-2 focus:ring-[var(--asf-green)]/20 outline-none text-sm" />
          </div>
       ...
       : <div className="space-y-2">{criadores.map(c => (
        <div key={c.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:border-gray-200 transition-colors">
          <div className="flex items-center gap-4 px-5 py-4 cursor-pointer" onClick={() => setExpId(expId === c.id ? null : c.id)}>
            <div className="w-11 h-11 rounded-full bg-[var(--asf-green)]/10 flex items-center justify-center flex-shrink-0 overflow-hidden">
              {c.avatar_url ? <img src={c.avatar_url} alt="" className="w-full h-full object-cover" /> : <span className="font-semibold text-[var(--asf-green)]">{c.nome.charAt(0)}</span>}
            </div>
            ...
            <div className="hidden sm:block text-sm text-[var(--asf-gray-medium)]">{c.cidade}{c.estado ? `, ${c.estado}` : ''}</div>
            <div className="hidden md:flex items-center gap-1 text-sm"><Star className="w-3.5 h-3.5 fill-[var(--asf-yellow)] text-[var(--asf-yellow)]" />{(typeof c.avaliacao_media === 'string' ? parseFloat(c.avaliacao_media) : c.avaliacao_media || 0).toFixed(1)}</div>
          ...
          {expId === c.id && <div className="px-5 pb-5 border-t border-gray-100 pt-4 bg-gray-50/50">
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
              ...
              <div><span className="text-[var(--asf-gray-medium)]">Coord.:</span> <b>{c.latitude ? `${Number(c.latitude).toFixed(4)}, ${Number(c.longitude).toFixed(4)}` : '—'}</b></div>
...
/* ── ESPÉCIES (com upload de imagem) ── */

function TabEspecies({ showToast }: { showToast: (m: string, t?: 'success' | 'error') => void }) {
  const { especies, isLoading, buscar, criar, atualizar, excluir, uploadImagem } = useAdminEspecies();
  const [search, setSearch] = useState('');
  ...
  const emptyForm = {
    ...
    dificuldade: 'intermediário', caixaIdeal: '', temperamento: '',
    conservacaoStatus: 'comum', biomas: [] as string[],
    melDescricao: '', melSabor: '', imagemUrl: '',
  };
  ...
  const edit = (e: AdminEspecie) => {
    setForm({
      id: e.id, nomeCientifico: e.nomeCientifico,
      nomesPopulares: (e.nomesPopulares || []).join(', '),
      familia: e.familia, tamanho: e.tamanho, producaoMel: e.producaoMel,
      comportamento: e.comportamento,
      dificuldade: e.manejo?.dificuldade || 'intermediário',
      caixaIdeal: e.manejo?.caixaIdeal || '',
      temperamento: e.manejo?.temperamento || '',
      conservacaoStatus: e.conservacao?.status || 'comum',
      biomas: e.biomas || [],
      melDescricao: e.mel?.descricao || '',
      melSabor: e.mel?.sabor || '',
      imagemUrl: e.imagem || '',
    });
  ...
  const submit = async () => {
    ...
    const payload: Partial<AdminEspecie> = {
      nomeCientifico: form.nomeCientifico,
      nomesPopulares: form.nomesPopulares.split(',').map(n => n.trim()).filter(Boolean),
      familia: form.familia, tamanho: form.tamanho, producaoMel: form.producaoMel,
      comportamento: form.comportamento, biomas: form.biomas,
      distribuicao: [], caracteristicas: [], fontes: [],
      imagem: form.imagemUrl || undefined,
      mel: { descricao: form.melDescricao, propriedades: [], sabor: form.melSabor },
      manejo: { dificuldade: form.dificuldade, caixaIdeal: form.caixaIdeal, temperamento: form.temperamento, cuidadosEspeciais: [] },
      conservacao: { status: form.conservacaoStatus, ameacas: [] },
    };
    const r = editingId ? await atualizar(editingId, payload) : await criar(payload);
    r.success ? (showToast(editingId ? 'Espécie atualizada!' : 'Espécie criada!'), reset()) : showToast(r.error || 'Erro ao salvar.', 'error');
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex-1 min-w-[200px] relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
          <input placeholder="Buscar espécie..." value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && buscar(search)} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 focus:border-[var(--asf-green)] outline-none text-sm" />
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl p-6 border-2 border-[var(--asf-green)]/20">
          ...
          <div className="space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div><label className="block text-sm text-[var(--asf-gray-medium)] mb-1">Caixa Ideal</label><input value={form.caixaIdeal} onChange={e => setForm({ ...form, caixaIdeal: e.target.value })} placeholder="INPA, PNN..." className="w-full px-3 py-2.5 rounded-xl border border-gray-200 outline-none text-sm" /></div>
              <div><label className="block text-sm text-[var(--asf-gray-medium)] mb-1">Temperamento</label><input value={form.temperamento} onChange={e => setForm({ ...form, temperamento: e.target.value })} placeholder="Dócil, Agressiva..." className="w-full px-3 py-2.5 rounded-xl border border-gray-200 outline-none text-sm" /></div>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div><label className="block text-sm text-[var(--asf-gray-medium)] mb-1">Descrição do Mel</label><textarea value={form.melDescricao} onChange={e => setForm({ ...form, melDescricao: e.target.value })} rows={2} className="w-full px-3 py-2.5 rounded-xl border border-gray-200 outline-none text-sm resize-none" /></div>
              <div><label className="block text-sm text-[var(--asf-gray-medium)] mb-1">Sabor do Mel</label><input value={form.melSabor} onChange={e => setForm({ ...form, melSabor: e.target.value })} placeholder="Doce, levemente ácido..." className="w-full px-3 py-2.5 rounded-xl border border-gray-200 outline-none text-sm" /></div>
            </div>
          </div>
      ...
      {isLoading ? <div className="flex justify-center py-12"><div className="animate-spin w-8 h-8 border-4 border-[var(--asf-green)] border-t-transparent rounded-full" /></div>
       : <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{especies.map(e => (
        <div key={e.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
          <div className="h-32 bg-[var(--asf-green)]/10 overflow-hidden">
            {e.imagem
              ? <img src={e.imagem} alt={e.nomesPopulares?.[0] || e.nomeCientifico} className="w-full h-full object-cover" onError={(ev) => { (ev.target as HTMLImageElement).style.display='none'; }} />
              : <div className="w-full h-full flex items-center justify-center"><Leaf className="w-8 h-8 text-[var(--asf-green)]/30" /></div>
            }
          </div>
          <div className="p-5">
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-[var(--asf-gray-dark)] truncate">{e.nomesPopulares?.[0] || e.nomeCientifico}</p>
                <p className="text-sm text-[var(--asf-gray-medium)] italic truncate">{e.nomeCientifico}</p>
              </div>
              <div className="flex gap-1 ml-2">
                <button onClick={() => edit(e)} className="p-1.5 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100"><Edit2 className="w-3.5 h-3.5" /></button>
                <button onClick={() => setDelId(e.id)} className="p-1.5 rounded-lg bg-red-50 text-red-500 hover:bg-red-100"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            </div>
...
/* ── CONTEÚDO DO SITE (banners, textos, imagens) ── */

function TabConteudo({ showToast }: { showToast: (m: string, t?: 'success' | 'error') => void }) {
  const { configs, isLoading, buscar, atualizar, uploadImagem } = useAdminSiteConfig();
  ...
  const getBanners = (): BannerItem[] => {
    const cfg = configs.find(c => c.chave === 'banners');
    return cfg ? (Array.isArray(cfg.valor) ? cfg.valor : []) : [];
  };

  const saveText = async (chave: string) => {
    const r = await atualizar(chave, editValor);
  ...
  const saveBanner = async (index: number) => {
    ...
    banners[index] = bannerForm;
    const r = await atualizar('banners', banners);
    r.success ? (showToast('Banner atualizado!'), setBannerEdit(null)) : showToast('Erro.', 'error');
  };

  const addBanner = async () => {
    const banners = [...getBanners(), { titulo: 'Novo Banner', descricao: 'Descrição do banner', cta: 'Saiba mais', link: '/', cor: 'green' as const, ativo: true }];
    const r = await atualizar('banners', banners);
    r.success ? showToast('Banner adicionado!') : showToast('Erro.', 'error');
  };

  const removeBanner = async (index: number) => {
    const banners = getBanners().filter((_, i) => i !== index);
    const r = await atualizar('banners', banners);
    r.success ? showToast('Banner removido.') : showToast('Erro.', 'error');
  };

  const toggleBanner = async (index: number) => {
    const banners = [...getBanners()];
    banners[index] = { ...banners[index], ativo: !banners[index].ativo };
    await atualizar('banners', banners);
  };

  // Separar configs por categoria
  const textos = configs.filter(c => c.tipo === 'texto');
  const imagens = configs.filter(c => c.tipo === 'imagem');
  const banners = getBanners();
  ...
  if (isLoading) return <div className="flex justify-center py-12"><div className="animate-spin w-8 h-8 border-4 border-[var(--asf-green)] border-t-transparent rounded-full" /></div>;
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-100">
        <div className="flex items-center justify-between mb-5">
          <h3 className="font-poppins font-semibold text-[var(--asf-gray-dark)] flex items-center gap-2"><Image className="w-5 h-5 text-[var(--asf-green)]" /> Banners do Carrossel</h3>
          <button onClick={addBanner} className="px-4 py-2 rounded-xl bg-[var(--asf-green)] text-white text-sm font-medium hover:bg-[var(--asf-green-dark)] flex items-center gap-2"><Plus className="w-4 h-4" /> Novo Banner</button>
        </div>
        <div className="space-y-3">
          {banners.map((banner, i) => (
            <div key={i} className={`rounded-xl border p-4 ${banner.ativo ? 'border-gray-200 bg-white' : 'border-gray-100 bg-gray-50 opacity-60'}`}>
              {bannerEdit === i ? (
                <div className="space-y-3">
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div><label className="block text-xs text-[var(--asf-gray-medium)] mb-1">Título</label><input value={bannerForm.titulo} onChange={e => setBannerForm({ ...bannerForm, titulo: e.target.value })} className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm outline-none focus:border-[var(--asf-green)]" /></div>
                    <div><label className="block text-xs text-[var(--asf-gray-medium)] mb-1">Texto do Botão</label><input value={bannerForm.cta} onChange={e => setBannerForm({ ...bannerForm, cta: e.target.value })} className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm outline-none focus:border-[var(--asf-green)]" /></div>
                  </div>
                  <div><label className="block text-xs text-[var(--asf-gray-medium)] mb-1">Descrição</label><input value={bannerForm.descricao} onChange={e => setBannerForm({ ...bannerForm, descricao: e.target.value })} className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm outline-none focus:border-[var(--asf-green)]" /></div>
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div><label className="block text-xs text-[var(--asf-gray-medium)] mb-1">Link</label><input value={bannerForm.link} onChange={e => setBannerForm({ ...bannerForm, link: e.target.value })} className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm outline-none focus:border-[var(--asf-green)]" /></div>
                    <div><label className="block text-xs text-[var(--asf-gray-medium)] mb-1">Cor</label>
                      <div className="flex gap-2">
                        {corOptions.map(c => (
                          <button key={c.value} onClick={() => setBannerForm({ ...bannerForm, cor: c.value as BannerItem['cor'] })} className={`w-8 h-8 rounded-lg ${c.class} ${bannerForm.cor === c.value ? 'ring-2 ring-offset-2 ring-[var(--asf-green)]' : 'opacity-50 hover:opacity-100'}`} title={c.label} />
                        ))}
                      </div>
                    </div>
                  </div>
                  <div className="flex justify-end gap-2 pt-2">
                    <button onClick={() => setBannerEdit(null)} className="px-3 py-1.5 rounded-lg bg-gray-100 text-sm">Cancelar</button>
                    <button onClick={() => saveBanner(i)} className="px-4 py-1.5 rounded-lg bg-[var(--asf-green)] text-white text-sm flex items-center gap-1"><Save className="w-3.5 h-3.5" /> Salvar</button>
                  </div>
                </div>
              ) : (
                <div className="flex items-center gap-4">
                  <div className={`w-3 h-12 rounded-full ${corOptions.find(c => c.value === banner.cor)?.class || 'bg-green-500'}`} />
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-[var(--asf-gray-dark)] truncate">{banner.titulo}</p>
                    <p className="text-sm text-[var(--asf-gray-medium)] truncate">{banner.descricao}</p>
                    <p className="text-xs text-[var(--asf-gray-medium)] mt-0.5">Link: {banner.link} · Botão: {banner.cta}</p>
                  </div>
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button onClick={() => toggleBanner(i)} className={`p-2 rounded-lg ${banner.ativo ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`} title={banner.ativo ? 'Desativar' : 'Ativar'}>
                      <Eye className="w-4 h-4" />
                    </button>
                    <button onClick={() => { setBannerForm(banner); setBannerEdit(i); }} className="p-2 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100"><Edit2 className="w-4 h-4" /></button>
                    <button onClick={() => removeBanner(i)} className="p-2 rounded-lg bg-red-50 text-red-500 hover:bg-red-100"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              )}
            </div>
          ))}
          {banners.length === 0 && <p className="text-center text-sm text-[var(--asf-gray-medium)] py-6">Nenhum banner configurado.</p>}
        </div>
      </div>
      ...
      {/* Textos Editáveis */}
      <div className="bg-white rounded-2xl p-6 border border-gray-100">
        <h3 className="font-poppins font-semibold text-[var(--asf-gray-dark)] mb-5 flex items-center gap-2"><FileText className="w-5 h-5 text-blue-600" /> Textos e Configurações</h3>
        <div className="space-y-3">
          {textos.map(cfg => (
            <div key={cfg.chave} className="flex items-start gap-4 py-3 border-b border-gray-100 last:border-0">
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[var(--asf-gray-dark)]">{cfg.descricao || cfg.chave}</p>
                <p className="text-xs text-[var(--asf-gray-medium)] mt-0.5">Chave: {cfg.chave}</p>
                {editChave === cfg.chave ? (
                  <div className="mt-2 flex gap-2">
                    <input
                      value={typeof editValor === 'string' ? editValor : JSON.stringify(editValor)}
                      onChange={e => setEditValor(e.target.value)}
                      className="flex-1 px-3 py-2 rounded-lg border border-gray-200 text-sm outline-none focus:border-[var(--asf-green)]"
                    />
                    <button onClick={() => saveText(cfg.chave)} className="px-3 py-2 rounded-lg bg-[var(--asf-green)] text-white text-sm"><Save className="w-4 h-4" /></button>
                    <button onClick={() => setEditChave(null)} className="px-3 py-2 rounded-lg bg-gray-100 text-sm"><X className="w-4 h-4" /></button>
                  </div>
                ) : (
                  <p className="text-sm text-[var(--asf-gray-dark)] mt-1 truncate">{typeof cfg.valor === 'string' ? cfg.valor : JSON.stringify(cfg.valor)}</p>
                )}
              </div>
              {editChave !== cfg.chave && (
                <button onClick={() => { setEditChave(cfg.chave); setEditValor(cfg.valor); }} className="p-2 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 flex-shrink-0"><Edit2 className="w-4 h-4" /></button>
              )}
            </div>
          ))}
        </div>
...
function TabNotificacoes({ showToast }: { showToast: (m: string, t?: 'success' | 'error') => void }) {
  ...
  const enviarParaTodos = async () => {
    if (!titulo.trim() || !mensagem.trim()) return showToast('Título e mensagem são obrigatórios.', 'error');
    setSending(true);

    if (!isSupabaseConfigured()) {
      setTimeout(() => { showToast('Notificação enviada! (modo offline)'); setSending(false); setTitulo(''); setMensagem(''); }, 1000);
      return;
    }

    try {
      // Buscar todos os user_ids
      const { data: users } = await supabase.from('criadores').select('user_id');
      if (users && users.length > 0) {
        ...
        showToast(`Notificação enviada para ${users.length} usuário(s)!`);
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl p-6 border border-gray-100">
        <h3 className="font-poppins font-semibold text-[var(--asf-gray-dark)] mb-5 flex items-center gap-2">
          <Bell className="w-5 h-5 text-[var(--asf-green)]" /> Enviar Notificação para Todos
        </h3>
        <div className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-[var(--asf-gray-medium)] mb-1">Título</label>
              <input value={titulo} onChange={e => setTitulo(e.target.value)} placeholder="Título da notificação" className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-[var(--asf-green)] outline-none text-sm" />
            </div>
            <div>
              <label className="block text-sm text-[var(--asf-gray-medium)] mb-1">Tipo</label>
              <select value={tipo} onChange={e => setTipo(e.target.value)} className="w-full px-3 py-2.5 rounded-xl border border-gray-200 outline-none text-sm">
          ...
          <div>
            <label className="block text-sm text-[var(--asf-gray-medium)] mb-1">Mensagem</label>
            <textarea value={mensagem} onChange={e => setMensagem(e.target.value)} rows={3} placeholder="Escreva a mensagem..." className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-[var(--asf-green)] outline-none text-sm resize-none" />
          </div>
          <button onClick={enviarParaTodos} disabled={sending} className="px-6 py-2.5 rounded-xl bg-[var(--asf-green)] text-white text-sm font-medium hover:bg-[var(--asf-green-dark)] flex items-center gap-2 disabled:opacity-50">
            {sending ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <Bell className="w-4 h-4" />}
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-yellow-700 mt-1">As notificações serão enviadas para todos os usuários cadastrados na plataforma. Use com moderação.</p>
          </div>
        </div>
      </div>
    </div>
...
function TabConfig() {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl p-6 border border-gray-100">
        <h3 className="font-poppins font-semibold text-[var(--asf-gray-dark)] mb-4 flex items-center gap-2"><Settings className="w-5 h-5 text-[var(--asf-green)]" /> Status do Sistema</h3>
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between py-3 border-b border-gray-100"><span className="text-sm text-[var(--asf-gray-medium)]">Ambiente</span><span className="px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700">{window.location.hostname === 'localhost' ? 'Desenvolvimento' : 'Produção'}</span></div>
          <div className="flex items-center justify-between py-3 border-b border-gray-100"><span className="text-sm text-[var(--asf-gray-medium)]">URL</span><span className="text-sm font-medium">{window.location.origin}</span></div>
          <div className="flex items-center justify-between py-3"><span className="text-sm text-[var(--asf-gray-medium)]">Versão</span><span className="text-sm font-medium">2.0</span></div>
        </div>
      </div>
      <div className="bg-white rounded-2xl p-6 border border-gray-100">
        <h3 className="font-poppins font-semibold text-[var(--asf-gray-dark)] mb-4">Links Úteis</h3>
        <div className="grid sm:grid-cols-2 gap-3">
          <a href="https://supabase.com/dashboard/project/vqfuxiynpbaywiddqhqy" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
            ...
            <div><p className="font-medium text-sm">Supabase Dashboard</p><p className="text-xs text-[var(--asf-gray-medium)]">Banco de dados, auth, storage</p></div>
            <ExternalLink className="w-4 h-4 text-gray-400 ml-auto" />
          </a>
          <a href="https://github.com/ASFBRASIL/asfcriadoresv1" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
          ...
          <a href="https://vercel.com/asfbrasil" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
            <div className="w-10 h-10 rounded-lg bg-black flex items-center justify-center"><span className="text-white text-lg font-bold">▲</span></div>
            <div><p className="font-medium text-sm">Vercel</p><p className="text-xs text-[var(--asf-gray-medium)]">Deploy e hospedagem</p></div>
            <ExternalLink className="w-4 h-4 text-gray-400 ml-auto" />
          </a>
          <a href="https://asfcriadoresv1.vercel.app" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
            <div className="w-10 h-10 rounded-lg bg-[var(--asf-green)]/10 flex items-center justify-center"><Eye className="w-5 h-5 text-[var(--asf-green)]" /></div>
            <div><p className="font-medium text-sm">Site em Produção</p><p className="text-xs text-[var(--asf-gray-medium)]">asfcriadoresv1.vercel.app</p></div>
            <ExternalLink className="w-4 h-4 text-gray-400 ml-auto" />
          </a>
        </div>
      </div>
...
type TabId = 'overview' | 'criadores' | 'especies' | 'conteudo' | 'avaliacoes' | 'notificacoes' | 'config';
...
export function Dashboard() {
  ...
  const tabs: { id: TabId; label: string; icon: LucideIcon }[] = [
    ...
    { id: 'especies', label: 'Espécies', icon: Leaf },
  ...
  return (
    <div className="min-h-screen pt-20 lg:pt-24 bg-[var(--asf-gray-light)]">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="container-asf section-padding py-5">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h1 className="text-xl lg:text-2xl font-poppins font-bold text-[var(--asf-gray-dark)]">Painel Administrativo</h1>
              <p className="text-sm text-[var(--asf-gray-medium)]">Gerencie todo o conteúdo, criadores, espécies e configurações da plataforma</p>
            </div>
            <div className="flex items-center gap-3">
              <Link to="/" className="px-3 py-1.5 rounded-lg bg-gray-100 text-sm font-medium text-[var(--asf-gray-medium)] hover:bg-gray-200 flex items-center gap-1.5">
                <Eye className="w-3.5 h-3.5" /> Ver site
              </Link>
              {user && <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[var(--asf-green)]/10">
                <div className="w-6 h-6 rounded-full bg-[var(--asf-green)] flex items-center justify-center overflow-hidden">
                  {criador?.avatar_url ? <img src={criador.avatar_url} alt="" className="w-full h-full object-cover" /> : <span className="text-xs font-semibold text-white">{criador?.nome?.charAt(0) || 'A'}</span>}
                </div>
                <span className="text-sm font-medium text-[var(--asf-green)]">{criador?.nome?.split(' ')[0] || 'Admin'}</span>
              </div>}
            </div>
          </div>
        </div>
      ...
      <div className="container-asf section-padding py-6">
        ...
        {tab === 'conteudo' && <TabConteudo showToast={show} />}
        {tab === 'avaliacoes' && <TabAvaliacoes showToast={show} />}
        {tab === 'notificacoes' && <TabNotificacoes showToast={show} />}
